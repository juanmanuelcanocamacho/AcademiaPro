...
export default function Sidebar({ session, settings, isOpen, setIsOpen }: { session: Session | null, settings?: any, isOpen: boolean, setIsOpen: (open: boolean) => void }) {
    ...
    return (
        <>
            ...
            {/* Sidebar Core */}
            <aside className={`fixed md:sticky top-0 left-0 h-[100dvh] shrink-0 w-64 md:w-60 bg-white border-r border-gray-100 flex flex-col z-50 select-none transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0 shadow-2xl md:shadow-none' : '-translate-x-full md:translate-x-0'}`}>
                {/* Logo */}
