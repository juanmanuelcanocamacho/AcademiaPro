...
export default function ExamModeSelector({ subjects }: { subjects: string[] }) {
            ...
            {subjects.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    {subjects.map((subject, i) => (
                        <Link
                            key={subject}
                            href={`/exam/${encodeURIComponent(subject)}?mode=${mode}&randomQ=${randomQ}&randomA=${randomA}`}
                            className="group bg-white border border-gray-200 rounded-2xl overflow-hidden flex flex-col hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
                        >
                            {/* Dark Tech Cover */}
                            <div className="relative h-44 flex flex-col justify-end p-6 bg-slate-900 overflow-hidden">
                                ...
                                <GraduationCap className={`absolute -right-6 -top-4 w-32 h-32 opacity-10 transition-transform duration-700 group-hover:scale-110 group-hover:-rotate-12 ${getSubjectAccentDark(i).split(' ')[1]}`} />

                                <div className={`relative z-10 border-l-4 pl-4 ${getSubjectAccentDark(i).split(' ')[0]}`}>
                            ...
                            {/* Card body */}
                            <div className="p-4 flex-1 flex items-center justify-between bg-white border-t border-gray-100">
